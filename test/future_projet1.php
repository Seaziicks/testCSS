...
for($g=2;$g<=6;$g++){
		...
		if(!is_null($competence['Description'.$g])){
			if($competence['Type_calcul'.$f]==1){
				...
				?>
				</span><span class="formule"><span class="<?php echo $competence['Impact'.$f.''];?>"><?php if($competence['Calcul'.$f.'a']!=0){echo $competence['Calcul'.$f.'a'];}if($competence['%'.$f.'']){echo '%';} ?></span> 
				<?php 
					if($competence['Calcul'.$f.'b']>1){
						if($competence['%'.$f.'']){
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>">+1%</span>/<?php echo ''.$competence['Calcul'.$f.'b'];?><span class="<?php echo $competence['Statistique'.$f.''];?>"><?php echo $competence['Statistique'.$f.'']; ?></span>)</span></span><br><br><br>
						<?php
						}else{
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>">+1</span>/<?php echo ''.$competence['Calcul'.$f.'b'];?><span class="<?php echo $competence['Statistique'.$f.''];?>"><?php echo $competence['Statistique'.$f.'']; ?></span>)</span></span><br><br>
				<?php 
						}
					}else{ 
						if($competence['%'.$f.'']){
						?>(<span class="<?php echo $competence['Impact'.$f.''];?>"><?php echo '+'.round(1/$competence['Calcul'.$f.'b']).'%'; ?></span>/<?php echo $competence['Statistique'.$f.'']; ?>)</span></span><br><br>
					<?php
						}else{
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>"><?php echo '+'.round(1/$competence['Calcul'.$f.'b']); ?></span>/<?php echo $competence['Statistique'.$f.'']; ?>)</span></span><br><br>
						<?php
