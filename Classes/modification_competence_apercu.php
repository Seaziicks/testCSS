...
for($g=2;$g<=6;$g++){
		...
		$impact="";
			...
			if($competence['Type_calcul'.$f]==1){
				...
				?>
				</span><span class="formule"><span ><span id="Calcul<?php echo $f?>a" class="<?php echo $competence['Impact'.$f.''];?>"><?php echo $competence['Calcul'.$f.'a'].'</span>';if($competence['Pourcentage'.$f.'']){echo '<span id="%'.$f.'">%';} ?></span> 
				<?php 
					if($competence['Calcul'.$f.'b']>1){
						if($competence['Pourcentage'.$f.'']){
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>">+1%</span>/<span id="Calcul<?php echo $f;?>b"><?php echo $competence['Calcul'.$f.'b'].'</span>';?><span class="<?php echo $competence['Statistique'.$f.''];?>"><?php echo '<span id="Statistique'.$f.'">'.$competence['Statistique'.$f.''].'</span>'; ?></span>)</span></span>
						<?php
						}else{
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>">+1</span>/<?php echo '<span id="Calcul'.$f .'b">'.$competence['Calcul'.$f.'b'].'</span>';?><span class="<?php echo $competence['Statistique'.$f.''];?>"><?php echo '<span id="Statistique'.$f.'">'.$competence['Statistique'.$f.''].'</span>'; ?></span>)</span></span>
				<?php 
						}
					}else{ 
						if($competence['Pourcentage'.$f.'']){
						?>(<span class="<?php echo $competence['Impact'.$f.''];?>"><?php echo '+<span id="Calcul'.$f.'b">'.round(1/$competence['Calcul'.$f.'b']).'</span>%'; ?></span>/<?php echo '<span id="Statistique'.$f.'">'.$competence['Statistique'.$f.''].'</span>'; ?>)</span></span>
					<?php
						}else{
							?>(<span class="<?php echo $competence['Impact'.$f.''];?>"><?php echo '+<span id="Calcul'.$f.'b">'.round(1/$competence['Calcul'.$f.'b']).'</span>'; ?></span>/<?php echo '<span id="Statistique'.$f.'">'.$competence['Statistique'.$f.''].'</span>'; ?>)</span></span>
						<?php
