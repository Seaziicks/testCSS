...
class Competence
{
    ...
    public $_Personnage;
    /* @var $_Effects CompetenceEffect[] */
    public $_Effects = [];
    /* @var $_LeveledEffects CompetenceEffect[] */
    public $_LeveledEffects = [];
    /* @var $_db PDO */
    public $_db;

    public function __construct(array $donnees, Personnage $Personnage, $db)
    {
        $this->_db = $db;
        $this->hydrate($donnees);
        $this->setPersonnage($Personnage);
        $this->getEffects();
        $this->getLeveledEffects();
    }
    ...
    public function getDescriptionComplete()
    {
        ...
        for ($g = 2; $g <= 6; $g++) {
            ...
            if (!is_null($this->getElement('Description', $g))) {
                ...
                if ($this->getElement('Type_calcul', $f) == 1) {
				...
				</span><span class="formule"><span
                            class="<?= $this->getImpact($f); ?>"><?php if ($this->getElement('Pourcentage', $f)) {
                            echo '%';
                        } ?></span>
                    <?php
                    if ($this->getCalculElement('b', $f) > 1) {
                        if ($this->getElement('Pourcentage', $f)) {
                            ?>(<span
                                    class="<?php echo $this->getImpact($f); ?>">+1%</span>/<?php echo '' . $this->getCalculElement('b', $f); ?>
                            <span
                                    class="<?php echo $this->getElement('Statistique', $f); ?>"><?php echo $this->getElement('Statistique', $f); ?></span>)</span></span>
                            <?php
                        } else {
                            ?>(<span
                                    class="<?php echo $this->getImpact($f); ?>">+1</span>/<?php echo '' . $this->getCalculElement('b', $f); ?>
                        ...
                        if ($this->getElement('Pourcentage', $f)) {
                            ?>(<span
                                    class="<?php echo $this->getImpact($f); ?>"><?php echo '+' . round(1 / $this->getCalculElement('b', $f)) . '%'; ?></span>/<?php echo $this->getElement('Statistique', $f); ?>)</span></span>
                            <?php
                        } else {
                            ?>(<span
                                    class="<?php echo $this->getImpact($f); ?>"><?php echo '+' . round(1 / $this->getCalculElement('b', $f)); ?></span>/<?php echo $this->getElement('Statistique', $f); ?>)</span></span>
                    ...
                    if (!is_null($this->getElement('Amplitude', $f))) {
                        ?>
                        <span class="<?php echo $this->getElement('Statistique', $f); ?>">+<span
                                    id="NombreAmplitude<?php echo $f; ?>"
                                    class=""><?php echo $this->getElement('Nombre_Amplitude', $f); ?></span>D<span
                                    id="Amplitude<?php echo $f; ?>"><?php echo $this->getElement('Amplitude', $f); ?></span></span>
                    <?php }
                    echo $this->getElement('Description', $g);
    ...
    public function getImpact($index)
    {
        ...
        echo $this->getCalculElement('b', $index);

        if ($this->getElement('Pourcentage', $index)) {
            $impact = '';
        } elseif ($this->getElement('Impact', $index) == "vie") {
            $impact = ' points de vie';
        } elseif ($this->getElement('Impact', $index) == "bouclier") {
            $impact = ' points de bouclier';
        } elseif ($this->getElement('Impact', $index) != "red" && $this->getCalculElement('a', $index) + (floor((float)$this->getStatistiqueValue($index) / $this->getCalculElement('b', $index))) > 2) {
            $impact = ' ' . $this->getElement('Impact', $index) . 's';
        } elseif ($this->getElement('Impact', $index) != "red") {
            $impact = ' ' . $this->getElement('Impact', $index);
        } elseif ($this->getElement('Impact', $index) == "red" && !$this->getElement('Pourcentage', $index)) {
    ...
    private function getLeveledEffects()
    {
        $competenceEffectsManager = new CompetenceEffectManager($this->_db);

        $this->_LeveledEffects = $competenceEffectsManager->getLeveledEffectListForCompetence(intval($this->_Id_Competence), $this->_Personnage);
    }
    ...
    public function getNewDescriptionComplete()
    {
        foreach ($this->_Effects as $effect) {
            if ($effect->getElement('typeCalcul') == 1) {
                ...
                $this->getDescriptionOfType2($effect);
            }
        }
    ...
    public function getDescriptionOfType1(CompetenceEffect $effect)
    {
        ...
        if ($effect->getCalculElement('B') > 1) {
            if ($effect->getElement('pourcentage')) {
                ?>(<span class="<?= $effect->_impact; ?>">+1%</span>/<?= '' . $effect->getCalculElement('B'); ?><span
                class="<?= $effect->getElement('statistique1'); ?>"><?= $effect->getElement('statistique1'); ?></span>)</span></span>
                <?php
            } else {
                ?>(<span class="<?= $effect->_impact; ?>">+1</span>/<?= '' . $effect->getCalculElement('B'); ?>
        ...
        if (!is_null($effect->getElement('amplitude'))) {
            ?>
            <span class="<?= $effect->getElement('statistique1'); ?>">+
        ...
        echo $effect->getDescriptionAfter();
    }

    public function getDescriptionOfType2(CompetenceEffect $effect)
    {
        echo $effect->getDescriptionBefore();
        ?><span class="voir"><span
        ...
        if ($effect->getCalculElement('B') > 1) {
            if ($effect->getElement('pourcentage')) {
                ?>(<span class="<?= $effect->_impact; ?>">+1%</span>/<?= $effect->_impact; ?>"><span
        ...
        if ($effect->getCalcul2Element('B') > 1) {
            if ($effect->getElement('pourcentage')) {
                ?>(<span class="<?= $effect->_impact; ?>">">+1%</span>/<?= '' . $effect->getCalcul2Element('B'); ?>
            ...
            if ($effect->getElement('pourcentage')) {
                ?>(<span
                        class="<?= $effect->_impact; ?>"><?php echo '+' . round(1 / $effect->getCalcul2Element('B')) . '%'; ?></span>/<?= $effect->getElement('statistique2') ?>)</span></span>
                <?php
            } else {
                ?>(<span
                        class="<?= $effect->_impact; ?>"><?php echo '+' . round(1 / $effect->getCalcul2Element('B')); ?></span>/<?= $effect->getElement('statistique2') ?>)</span></span>
        ...
        if (!is_null($effect->getElement('amplitude'))) {
            ?>
            <span class="<?= $effect->getElement('statistique1'); ?>">+<span
    ...
    public function getLeveledEffectsDescriptions()
    {
        $i = 1;
        foreach ($this->_LeveledEffects as $effect) {
            if ($this->_Niveau >= ($i * 3)) {
                ?><span class="activé"><?php
            } else {
                // Hide more and more the unreached capacities. More far you are, more hidden the description will be.
                ?><span class="nonactivé" style="opacity: <?= pow(0.42, ($i - ($this->_Niveau / 3)))?>;"><?php
            }
            if (!empty($effect)) {
                echo '<br><br>' . $effect->_descriptionBefore;
                if (!is_null($effect->_typeCalcul)) {
                    if ($effect->_typeCalcul == 1) {
                        ?><span class="<?= $effect->_impact; ?>"> <?php
                        echo $effect->getCalcul().$effect->getImpact(1);
                    } elseif ($effect->_typeCalcul == 2) {
                        ?><span class="<?= $effect->_impact; ?>"><?php
                        echo $effect->getCalcul2().$effect->getImpact(2);

                }
                    echo $effect->_descriptionAfter;
                    ?></span><?php
                }
            }
            ?></span><?php
