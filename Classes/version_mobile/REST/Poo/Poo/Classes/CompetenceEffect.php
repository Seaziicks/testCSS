...
class CompetenceEffect
{
    ...
    public function __construct(array $donnees, Personnage $Personnage)
    {
    ...
    {
        return ($this->getCalculElement('A') +
            (floor((float)$this->getStatistiqueValue(1) / $this->getCalculElement('B'))) +
            $this->getCalcul2Element('A') +
            (floor((float)$this->getStatistiqueValue(2) / $this->getCalcul2Element('B'))));
    }
    ...
    public function getCalcul2Element($secondPartElement)
    {
        $retour = '_calcul2' . $secondPartElement;
        return $this->$retour;
    }

    public function getImpact(int $index)
    {
        $impact = null;
        ...
        } elseif ($index == 1 && $this->getElement('impact') != "red" && $this->getCalculElement('A') + (floor((float)$this->getStatistiqueValue(1) / $this->getCalculElement('B'))) > 2) {
            $impact = ' ' . $this->getElement('impact') . 's';
        } elseif ($index == 2 && $this->getElement('impact') != "red" && ($this->getCalculElement('A') + (floor((float)$this->getStatistiqueValue(1) / $this->getCalculElement('B'))) + $this->getCalcul2Element('A') + (floor((float)$this->getStatistiqueValue(2) / $this->getCalcul2Element('B'))) ) > 2) {
    ...
    public function appliquerEffetCompetenceAvecBonusGeneraux(PDO $bdd, Personnage $launcher, Personnage $receiver, BonusCombat $bonusCombatLauncher, BonusCombat $bonusCombatReceiver)
    {
        switch (true) {
