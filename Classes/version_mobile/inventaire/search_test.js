(function() {
	...
	function getResults(keywords) { // Effectue une requête et récupère les résultats
    	...
    	xhr.addEventListener('readystatechange', function() {
				...
				// alert(test.Competence1);
				// alert(test['Competence'+i]);
	...
	function displayResults(response) { // Affiche les résultats d'une requête
	    ...
	    if (response.length) { // On ne modifie les résultats que si on en a obtenu
	        ...
	        var responseLen = Object.keys(JSON.parse(response)).length/3;
			// alert(responseLen);
	        results.innerHTML = ''; // On vide les résultats
	
	        for (var i = 0 ; i < responseLen ; i++) {
	
            	var divNom = results.appendChild(document.createElement('div'));
            	divNom.innerHTML = JSON.parse(response)['Competence'+(i+1)];
				divNom.id='Competence_recherche_'+(i+1);
				divNom.classList.add('Competence_recherche');
				
				if(JSON.parse(response)['Image'+(i+1)]){
					var divImage =  divNom.appendChild(document.createElement('div'));
					var img = divImage.appendChild(document.createElement('img'));
					// alert(JSON.parse(response)['Image'+(i+1)]);
					img.src='../../'+JSON.parse(response)['Image'+(i+1)];
					img.id='Image_recherche_'+(i+1);
					divImage.classList.add('Image_recherche');
				}
				
				if(JSON.parse(response)['PA'+(i+1)]){
					var divPA = divNom.appendChild(document.createElement('div'));
					divPA.innerHTML = 'Coût en PA : '+JSON.parse(response)['PA'+(i+1)];
					divPA.id='PA_recherche_'+(i+1);
