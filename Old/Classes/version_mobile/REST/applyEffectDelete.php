...
$personnagesInfos = $bdd->query('SELECT *
	...
	while($resultat = $competences->fetch()){
		...
		$fin = $bdd->query('SELECT ID_Competence
					FROM competence
		...
		if(isset($competence['ID_Competence']) && !is_null($competence['ID_Competence'])){$tableau = $competence['ID_Competence'];}else{$tableau = NULL;}
