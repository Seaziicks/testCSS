...
for ($competenceIDNumber = 0; $competenceIDNumber < 265; $competenceIDNumber++) {
    ...
    */
    ...
    for($k = 1 ; $k < 4 ; $k++)
    {
        $pourcentage = is_null($donnees['PourcentageBis']) ? 0 : 1;
        $descriptioni = str_replace("'", "\'", $donnees['Effet'.$k]);
        $descriptioni2 = str_replace("'", "\'", $donnees['Effet'.$k.'Bis']);
        $CalculBis2a = is_null($donnees['CalculBis2a']) ? 0 : $donnees['CalculBis2a'];

        echo "<br/><br/><br/>Le champ problématique en question (effet n°$k) : $descriptioni2<br/><br/><br/>";

        if($donnees['NumEffet'] == $k) {
            if ($donnees['Type_calculBis1'] == 1) {

                $bdd->exec('INSERT INTO competenceeffect(effectOrder, idCompetence, actionType, effectType, applicationType, niveauRequis, descriptionBefore, descriptionAfter,
                             typeCalcul, calcul1A, calcul1B, calcul2A, calcul2B, amplitude, nombreAmplitude, statistique1, statistique2, impact, pourcentage,
                             numberOfUse, numberOfTurn, numberOfFight, numberOfTarget, accumulationType, numberOfAccumulation, successiveAccumulation, linkedEffect) 
                VALUES(' . $j . ', ' . $donnees['Id_Competence'] . ', 1, NULL, 2, '.(3*$k).', \''. $descriptioni .'\', \''. $descriptioni2 .'\', 
                ' . $donnees['Type_calculBis1'] . ', ' . $donnees['CalculBis1a'] . ', ' . $donnees['CalculBis1b'] . ', NULL, NULL, 
                NULL, NULL, \'' . $donnees['StatistiqueBis1'] . '\', NULL, 
                \'' . $donnees['ImpactBis'] . '\', ' . $pourcentage . ', NULL, NULL, NULL, 0, 0, 0, 0, 0)');

            } elseif ($donnees['Type_calculBis1'] == 2) {
                echo 'INSERT INTO competenceeffect(effectOrder, idCompetence, actionType, effectType, applicationType, niveauRequis, descriptionBefore, descriptionAfter,
                             typeCalcul, calcul1A, calcul1B, calcul2A, calcul2B, amplitude, nombreAmplitude, statistique1, statistique2, impact, pourcentage,
                             numberOfUse, numberOfTurn, numberOfFight, numberOfTarget, accumulationType, numberOfAccumulation, successiveAccumulation, linkedEffect) 
                VALUES(' . $j . ', ' . $donnees['Id_Competence'] . ', 1, NULL, 2, '.(3*$k).', \''.$descriptioni.'\', \'' . $descriptioni2 . '\', 
                ' . $donnees['Type_calculBis1'] . ', ' . $donnees['CalculBis1a'] . ', ' . $donnees['CalculBis1b'] . ', ' . $CalculBis2a . ', 
                ' . $donnees['CalculBis2b'] . ', NULL, NULL, \'' . $donnees['StatistiqueBis1'] . '\', 
                \'' . $donnees['StatistiqueBis2'] . '\', \'' . $donnees['ImpactBis'] . '\', ' . $pourcentage . ', NULL, NULL, NULL, 0, 0, 0, 0, 0)';
                $bdd->exec('INSERT INTO competenceeffect(effectOrder, idCompetence, actionType, effectType, applicationType, niveauRequis, descriptionBefore, descriptionAfter,
                             typeCalcul, calcul1A, calcul1B, calcul2A, calcul2B, amplitude, nombreAmplitude, statistique1, statistique2, impact, pourcentage,
                             numberOfUse, numberOfTurn, numberOfFight, numberOfTarget, accumulationType, numberOfAccumulation, successiveAccumulation, linkedEffect) 
                VALUES(' . $j . ', ' . $donnees['Id_Competence'] . ', 1, NULL, 2, '.(3*$k).', \''.$descriptioni.'\', \'' . $descriptioni2 . '\', 
            ...
            $bdd->exec('INSERT INTO competenceeffect(effectOrder, idCompetence, actionType, effectType, applicationType, niveauRequis, descriptionBefore, descriptionAfter,
                             typeCalcul, calcul1A, calcul1B, calcul2A, calcul2B, amplitude, nombreAmplitude, statistique1, statistique2, impact, pourcentage,
                             numberOfUse, numberOfTurn, numberOfFight, numberOfTarget, accumulationType, numberOfAccumulation, successiveAccumulation, linkedEffect) 
            VALUES(' . $j . ', ' . $donnees['Id_Competence'] . ', 1, NULL, 2, '.(3*$k).', \''.$descriptioni.'\', \'\', 
            NULL, NULL, NULL, NULL, NULL, 
            NULL, NULL, NULL, NULL, 
            NULL, ' . $pourcentage . ', NULL, NULL, NULL, 0, 0, 0, 0, 0)');
        }
        $j++;
    }
}