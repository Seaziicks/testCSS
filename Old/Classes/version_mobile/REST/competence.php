...
$competence=$competenceInfos->fetch();				
	...
	for($g=2;$g<=6;$g++){
		...
		$impact="";
			...
			if($competence['Type_calcul'.$f]==1){
				...
				if($competence['Pourcentage'.$f.'']){$impact='%';$Impact[$f]="Pourcentage";}
				elseif($competence['Impact'.$f.''] == "vie"){$impact=' points de vie'; $Impact[$f]="Soin";}
				elseif($competence['Impact'.$f.''] == "bouclier" ){$impact=' points de bouclier';$Impact[$f]="Bouclier";}
				elseif($competence['Impact'.$f.''] != "red" && $competence['Calcul'.$f.'a']+(floor((float)${'stat'.$f}/$competence['Calcul'.$f.'b'])).''.$impact>2){$impact=' '.$competence['Impact'.$f.''].'s';$Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['Impact'.$f.''] != "red"){$impact=' '.$competence['Impact'.$f.''];$Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['Impact'.$f.''] == "red" && !$competence['Pourcentage'.$f.'']){$impact=' points de dégâts';$Impact[$f]="Dégâts";}
				...
				{$impact=' '.$competence['Impact'.$f.''].'s'; $Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['Impact'.$f.''] != "red"){$impact=' '.$competence['Impact'.$f.'']; $Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['Impact'.$f.''] == "red" && !$competence['Pourcentage'.$f.'']){$impact=' points de dégâts'; $Impact[$f]="Dégâts";}
				...
				if($competence['PourcentageBis']){$impact='%';$Impact[$f]="Pourcentage";}
				elseif($competence['ImpactBis'] == "vie"){$impact=' points de vie'; $Impact[$f]="Soin";}
				elseif($competence['ImpactBis'] == "bouclier" ){$impact=' points de bouclier'; $Impact[$f]="Bouclier";}
				elseif($competence['ImpactBis'] != "red" && isset($statBis1) && $competence['CalculBis1b']!=0 && $competence['CalculBis1a']+(floor((float)$statBis1/$competence['CalculBis1b']))>2)
				{$impact=' '.$competence['ImpactBis'].'s'; $Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['ImpactBis'] != "red"){$impact=' '.$competence['ImpactBis']; $Impact[$f]=$competence['Impact'.$f.''];}
				elseif($competence['ImpactBis'] == "red" && !$competence['Pourcentage'.$f.'']){$impact=' points de dégâts'; $Impact[$f]="Dégâts";}
	...
	for ($k=1;$k<=6;$k++){
		if(isset($competence['Description'.$k.'']) && !is_null($competence['Description'.$k.''])){$réponse['Description'.$k.''] = preg_replace('#\<(.*)\>#sU',"",$competence['Description'.$k.'']);}else{$réponse['Description'.$k.''] = NULL;}
		if(isset($resultat[$k]) && !is_null($resultat[$k])){$réponse['Resultat'.$k.''] = $resultat[$k];}else{$réponse['Resultat'.$k.''] = NULL;}
		if(isset($Impact[$k]) && !is_null($Impact[$k])){$réponse['Impact'.$k.''] = $Impact[$k];}else{$réponse['Impact'.$k.''] = NULL;}
	...
	for ($k=1;$k<=3;$k++){
		if(isset($competence['Effet'.$k.'']) && !is_null($competence['Effet'.$k.'']) && $competence['Niveau']>=3*$k){$réponse['Effet'.$k.''] = preg_replace('#\<(.*)\>#sU',"",$competence['Effet'.$k.'']);}else{$réponse['Effet'.$k.''] = NULL;}
	...
	if(isset($competence['Entete']) && !is_null($competence['Entete'])){$réponse['Entete'] = $competence['Entete'];}else{$réponse['Entete'] = NULL;}
	if(isset($competence['Niveau']) && !is_null($competence['Niveau'])){$réponse['Niveau'] = $competence['Niveau'];}else{$réponse['Niveau'] = NULL;}
	if(isset($competence['Cout_En_PA']) && !is_null($competence['Cout_En_PA'])){$réponse['Cout_En_PA'] = $competence['Cout_En_PA'];}else{$réponse['Cout_En_PA'] = NULL;}
	if(isset($competence['Portée']) && !is_null($competence['Portée'])){$réponse['Portée'] = $competence['Portée'];}else{$réponse['Portée'] = NULL;}
	if(isset($competence['Image']) && !is_null($competence['Image'])){$réponse['Image'] = $competence['Image'];}else{$réponse['Image'] = NULL;}
