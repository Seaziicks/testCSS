	...
	if(isset($_SESSION['pseudo'])){
		...
		if (isset($id_groupe) && $id_groupe == 0) {
			...
			$competences = $bdd->query('SELECT *
											...
											and c.niveau > 0
											and a.personnage="'.$_POST['personnage'].'"');
			...
			$MAJpersonnage= $bdd->query('UPDATE personnage SET PA_Actuel = PA, PM_Actuel = PM WHERE Libellé = "'.$_POST['personnage'].'"');
			
			//Cas du démoniste
			if($_POST['personnage']=='Démoniste'){
				$MAJressourcepersonnage= $bdd->query('UPDATE personnage SET Ressource_Actuelle = Ressource_Actuelle+Ressource*0.1, PM_Actuel = PM WHERE Libellé = "'.$_POST['personnage'].'"');
			...
			if($_POST['personnage']=='Démoniste'){
			$MAJressourcepersonnage->closeCursor();
			}
			$MAJpersonnage->closeCursor();
